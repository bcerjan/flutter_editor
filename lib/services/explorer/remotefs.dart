...
class RemoteFs<T extends RemoteConnection> extends ExplorerBackend<T> {
  ...
  void fileEventMessageHandler(ServerMessage msg) async {
    if (msg.type == ServerMessageType.fileSystemEvents) {
      // server format is: { events: FileEvent[] }
      final List<FileEvent> events =
          (msg.content['events'] as List<Map<String, dynamic>>)
              .map((e) => FileEvent.fromJson(e))
              .toList();
      for (final event in events) {
        if (event.type == FileChangeType.Changed) {
          // Can't handle this here, needs to be in app provider
        } else if (event.type == FileChangeType.Created) {
          //reload the path:
          loadPath(_path.normalize(event.uri.path));
        } else if (event.type == FileChangeType.Deleted) {
          for (final l in listeners) {
            l.onDelete({'path': _path.normalize(event.uri.path)});
          }
        }
      }
